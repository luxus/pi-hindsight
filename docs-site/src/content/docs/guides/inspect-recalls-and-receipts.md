...
Enable `recall.storeLastRecall: true` and read the sidecar at the configured `recall.lastRecallPath` (default `.pi/hindsight/last-recall.json`). There is no public slash command for last-recall. Esc cancellation does not write a success or failure snapshot.
