...
Amendment (2026-07-10): [ADR-005](/pi-hindsight/architecture/adr/005-domain-banks-and-agent-first-surface/) changes the long-term default **Project Bank** shape from “one path-derived bank per repo” toward a **coding domain bank + project tags**, and renames the user-facing cross-project bank path toward **Life Bank**. This ADR’s decision **(b)** still holds: Life/User Bank stays **opt-in**, automatic retain never writes there, and dual-bank recall budgets remain. Topology and agent-first surface details live in ADR-005.
